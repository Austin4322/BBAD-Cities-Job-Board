...
const TITLE_EXCLUDES = [
  // seniority (fellowship is early-career)
  "senior", "staff engineer", "principal", "director", "vp", "vice president", "chief", "head of",
  // trades / industrial / facilities — don't spend credits on these
  "welder", "fabricator", "machinist", "mailroom", "warehouse", "forklift", "assembler",
  "driver", "mechanic", "electrician", "plumber", "hvac", "janitor", "custodian", "technician",
  "maintenance", "laborer", "installer", "carpenter", "roofer", "landscaper", "painter",
  // food service / retail floor
  "cook", "chef", "barista", "bartender", "dishwasher", "cashier", "crew member",
  // clinical / care
  "nurse", "cna", "medical assistant", "dental", "caregiver", "phlebotomist",
];
...
export async function fetchTheirstackJobs() {
  ...
  // THEIRSTACK_FORCE=true bypasses the gate for a manual re-pull.
  const force = process.env.THEIRSTACK_FORCE === "true";
  ...
  if (!force && daysSinceRun < interval) {
    ...
    try {
      const res = await fetch(API, {
        ...
        body: JSON.stringify({
          ...
          // On a forced pull, drop the discovered_at floor so jobs from any
        // erased/lost previous pull can be recovered.
        ...(state.lastRun && !force ? { discovered_at_gte: state.lastRun } : {}),
      ...
      for (const j of returned) {
        ...
        if (matchCity(loc) !== cityId) { dropped++; continue; }
        jobs.push({
          id: `ts-${j.id}`,
          title: j.job_title,
          ...
          remote: false, // server-side filter already excludes remote
          source: "TheirStack",
          salary:
            j.salary_string ||
      ...
      console.log(`  · ${city.label}: ${returned.length} returned, ${dropped} failed strict location check`);
    } catch (err) {
