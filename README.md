...
scripts/
  ...
  build.js           The orchestrator (steps 1–4 above). Dedupe rules: within
                     a build, jobs collapse on a normalized company+title key
                     (direct ATS records beat aggregator copies). Across days,
                     ATS jobs never duplicate because boards are re-fetched in
                     full and matched by stable IDs — and a job missing from
                     its board drops off (role filled). TheirStack jobs are
                     never re-fetched (discovered_at_gte), so previous pulls
                     are carried forward until they age past MAX_AGE_DAYS.
                     Reuses yesterday's Claude classifications by job ID, so
...
3. **Check TheirStack has pulled** — on the free tier it runs weekly, so between pulls new listings come only from your curated list. The build log prints its credit balance, next pull time, and per-city counts of jobs returned vs dropped by the strict location check. To pull immediately: Actions → "Refresh jobs daily" → Run workflow → tick **Force a TheirStack pull now**. A forced pull ignores the cadence AND re-fetches the full window (no discovered_at floor), so it also recovers jobs from any lost pull.
